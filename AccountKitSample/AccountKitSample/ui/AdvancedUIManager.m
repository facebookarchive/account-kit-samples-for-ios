...
- (instancetype)initWithTheme:(AKFTheme *)theme
            confirmButtonType:(AKFButtonType)confirmButtonType
                 ...
                 textPosition:(AKFTextPosition)textPosition
{
  if ((self = [super init])) {
    ...
    self.theme = theme;
    self.theme.statusBarStyle = UIStatusBarStyleDefault;
  }
  return self;
}
...
- (UIView *)actionBarViewForState:(AKFLoginFlowState)state
{
  ...
  [view setUpdateSafeAreaInsets:true];
...
- (UIView *)headerViewForState:(AKFLoginFlowState)state
{
  if (state == AKFLoginFlowStateError) {
    ...
    return [self _viewWithText:errorMessage intrinsicHeight:120.0];
  }
