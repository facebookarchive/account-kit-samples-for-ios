...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _label.textColor = [UIColor colorWithRed:244.0/255.0 green:171.0/255.0 blue:211.0/255.0 alpha:1.0];
    ...
    _outlineLayer.lineDashPattern = @[@2.0, @2.0];
    _outlineLayer.lineWidth = 1.0;
    _outlineLayer.opaque = NO;
    _outlineLayer.strokeColor = [UIColor colorWithRed:244.0/255.0 green:171.0/255.0 blue:211.0/255.0 alpha:1.0].CGColor;
    [self.layer addSublayer:_outlineLayer];

    self.backgroundColor = [UIColor colorWithRed:251.0/255.0 green:229.0/255.0 blue:242.0/255.0 alpha:1.0];
...
-(void)safeAreaInsetsDidChange {
  if (_updateSafeAreaInsets) {
    if (@available(iOS 11.0, *)) {
      UIEdgeInsets newContentInset = self.contentInset;
      if (self.safeAreaInsets.top > newContentInset.top) {
        _intrinsicHeight += (self.safeAreaInsets.top - newContentInset.top);
        newContentInset.top = self.safeAreaInsets.top;
...
- (void)setUpdateSafeAreaInsets:(BOOL)updateSafeAreaInsets
{
  if (_updateSafeAreaInsets != updateSafeAreaInsets) {
    _updateSafeAreaInsets = updateSafeAreaInsets;
  }
}
